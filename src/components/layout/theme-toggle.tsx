...
const OPTIONS: { value: Theme; label: string; icon: typeof Sun; hint: string }[] = [
  ...
  // "light" is also what an unset preference resolves to — see DEFAULT_THEME.
];

/**
 * Light / dark / system switcher.
 *
 * Three options rather than a two-way flip: the site paints light by default,
 * so visitors need an explicit way to dark *and* a way to hand the
 * decision to their operating system.
 *
 * Until `mounted`, the icon renders in a fixed state — the sun, matching the
 * light default. The server cannot know the stored preference, so painting the
